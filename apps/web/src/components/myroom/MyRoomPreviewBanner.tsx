/**
 * MyRoom が「非公開中」のときに管理者だけに表示する警告バナー。
 *
 * 非公開でも管理者は閲覧できる (開発中の動作確認のため) が、
 * その状態だと管理者からは公開時とまったく同じ画面に見えてしまい、
 * 「公開したつもりで実は非公開のままだった」あるいは逆に
 * 「非公開のつもりが公開されていた」という事故が起きうる。
 * それを防ぐため、非公開中は画面の先頭にこのバナーを出す。
 *
 * ゲームの GamePreviewBanner と同じ役割・同じ見た目に揃えている
 * (運営が別の意味だと誤解しないようにするため)。
 */
import Link from 'next/link';
import { EyeOff } from 'lucide-react';

export function MyRoomPreviewBanner() {
  return (
    <div
      role="status"
    ...
    >
      ...
      <p className="text-sm font-semibold text-amber-900">
        ReiRieRoom は現在「非公開」です（管理者のみ表示中）
      </p>
      <p className="text-xs text-amber-800">
        一般の会員にはこのページは表示されません（404 になります）。
      </p>
      <Link
